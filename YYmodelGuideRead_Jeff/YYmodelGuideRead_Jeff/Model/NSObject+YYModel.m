...
static force_inline NSNumber *YYNSNumberCreateFromID(__unsafe_unretained id value) {
    ...
    if ([value isKindOfClass:[NSString class]]) {
        ...
        if ([(NSString *)value rangeOfCharacterFromSet:dot].location != NSNotFound) {
            ...
            double num = atof(cstring);     // double atof(const char *nptr); 将数字从String转化为double
            if (isnan(num) || isinf(num)) return nil;
            return @(num);
        } else {
            // value传入的值是5788转换为NSNumber数字类型
...
@interface _YYModelPropertyMeta : NSObject {
    @package                                                        //"name"这个属性举例子
    NSString *_name;             ///< property's name                   "name"
    YYEncodingType _type;        ///< property's type                   655374
    YYEncodingNSType _nsType;    ///< property's Foundation type        "YYEncodingTypeNSString"
    BOOL _isCNumber;             ///< is c number type                  NO
    Class _cls;                  ///< property's class, or nil          NSString    属性的类型
    Class _genericCls;           ///< container's generic class, or nil if threr's no generic class
    SEL _getter;                 ///< getter, or nil if the instances cannot respond        get方法
    SEL _setter;                 ///< setter, or nil if the instances cannot respond        set方法
    BOOL _isKVCCompatible;       ///< YES if it can access with key-value coding            能否通过 KVC
    BOOL _isStructAvailableForKeyedArchiver; ///< YES if the struct can encoded with keyed archiver/unarchiver      NO
    BOOL _hasCustomClassFromDictionary; ///< class/generic class implements +modelCustomClassForDictionary:         NO      //自定义容器flag
    
    NSString *_mappedToKey;      ///< the key mapped to
    NSArray *_mappedToKeyPath;   ///< the key path mapped to (nil if the name is not key path)
    NSArray *_mappedToKeyArray;  ///< the key(NSString) or keyPath(NSArray) array (nil if not mapped to multiple keys)           @"bookID" : @[@"id",@"ID",@"book_id"]
...
+ (instancetype)metaWithClassInfo:(YYClassInfo *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    //确定属性是 C type（基本数据类型） 还是 oc 属性（自定义属性为 unknown）
    ...
    if ((meta->_type & YYEncodingTypeMask) == YYEncodingTypeStruct) {
        ...
        if ([types containsObject:propertyInfo.typeEncoding]) {
            meta->_isStructAvailableForKeyedArchiver = YES; //如果是结构体，则立 flag。
        }
    }
    
    meta->_cls = propertyInfo.cls;
    
    //meta->_cls && meta->_nsType == YYEncodingTypeNSUnknown的意思是自定义属性判断
    if (generic) {
        meta->_hasCustomClassFromDictionary = [generic respondsToSelector:@selector(modelCustomClassForDictionary:)];
    } else if (meta->_cls && meta->_nsType == YYEncodingTypeNSUnknown) {
        meta->_hasCustomClassFromDictionary = [meta->_cls respondsToSelector:@selector(modelCustomClassForDictionary:)];
    }
    //注意此处省略了一个 else 。有些属性数据这两个都不经过。
    ...
    if (meta->_getter && meta->_setter) {
        /*
         KVC invalid type:          不是很懂，经过测试long可以，double最后一位会有精度缺失。
         long double
...
@interface _YYModelMeta : NSObject {
    ...
    BOOL _hasCustomWillTransformFromDictionary;     //在模型转换前被调用
    BOOL _hasCustomTransformFromDictionary;         //在模型转换最后被调用
    BOOL _hasCustomTransformToDictionary;           //模型转字典最后被调用
    BOOL _hasCustomClassFromDictionary;             //可以根据dic来指向子类中的一个
}
@end
...
- (instancetype)initWithClass:(Class)cls {
    ...
    // Get black list       如果方法实现了代理，则获取方法里的属性。
    NSSet *blacklist = nil;
    ...
    // Get container property's generic class       容器类对象转换
    // 为什么不是直接拿来用，而有这么多操作呢？
    ...
    if ([cls respondsToSelector:@selector(modelContainerPropertyGenericClass)]) {
        ...
        if (genericMapper) {
            ...
            [genericMapper enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                ...
                Class meta = object_getClass(obj);      //获取 value 内的类。 obj = [YYAuthor class].so meta = 元类
                if (!meta) return;
    ...
    /*
     ...
     _YYModelPropertyMeta：对属性的进一步封装，有一些 map。
...
//            NSLog(@"genericMapper[propertyInfo.name] = %@ -- name = %@",genericMapper[propertyInfo.name],propertyInfo.name);
    ...
    // create mapper
    NSMutableDictionary *mapper = [NSMutableDictionary new];        //属性名字和_YYModelPropertyMeta一一对应.包括所有映射
...
//        NSLog(@"");
        ...
        propertyMeta->_next = mapper[name] ?: nil;          //所以一般的情况都是 nil。    除非有自定义的映射
...
//        NSLog(@"");
    ...
    if (keyPathPropertyMetas) _keyPathPropertyMetas = keyPathPropertyMetas;     //@"desc" : @"ext.desc",
...
+ (instancetype)metaWithClass:(Class)cls {
    ...
    _YYModelMeta *meta = CFDictionaryGetValue(cache, (__bridge const void *)(cls));     //调用CFDictionaryGetValue的GetValue
    dispatch_semaphore_signal(lock);
    
    //meta不存在 或者 升级标志位为真 时 meta 重新初始化。
    if (!meta || meta->_classInfo.needUpdate) {
        meta = [[_YYModelMeta alloc] initWithClass:cls];        //通过 类 获取大量和 类 相关的信息。
        if (meta) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
            CFDictionarySetValue(cache, (__bridge const void *)(cls), (__bridge const void *)(meta));
            //调用CFDictionaryGetValue的SetValue
            dispatch_semaphore_signal(lock);
        }
    }
    return meta;
...
static void ModelSetValueForProperty(__unsafe_unretained id model,
    ...
    /*
     1.C type
     2.Foundation type        YYEncodingTypeNSUnknown = 0
     3. 自定义容器                  直接走 get 方法，如果没有获取到值。 字典转模型。
     此处    1.属性getter方法获取。（自定属性为类）
            2.如果失败，则利用meta->cls 获取类信息。初始化一个出来。发生字典转模型。    然后再 setter
        ...
        switch (meta->_type & YYEncodingTypeMask) {
            case YYEncodingTypeObject: {
                if (isNull) {
                    ...
                    NSObject *one = nil;
                    //看看有没有自定义的 get 方法？ get也是希望 get 到实例对象的。
                    if (meta->_getter) {
                    ...
                    if (one) {
                        ...
                        one = [cls new];
                        [one modelSetWithDictionary:value]; //字典转模型可以生成 set 方法。
                        //modelSetWithDictionary已经转好了。但需要把转好的 author(one) 赋值到 book上去。
...
typedef struct {
    void *modelMeta;  ///< _YYModelMeta             本身的大量信息，相关属性，map
    void *model;      ///< id (self)                本身
    void *dictionary; ///< NSDictionary (json)      json转换成的 dict
} ModelSetContext;
...
//    NSLog(@"---key %@ value %@",_key,_value);
    ...
    __unsafe_unretained id model = (__bridge id)(context->model);   //model = self(实例对象)
...
//            NSLog(@"_setter = %@ ",NSStringFromSelector(propertyMeta->_setter));
            //用objc_msgSend给   模型   发送setter消息  参数为 value。
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    if ([model isKindOfClass:[NSDictionary class]]) {
        ...
        //可以被转换为 Json 对象的字典。    属于系统检测      里面的都是foundtion框架的（无自定义属性。
        if ([NSJSONSerialization isValidJSONObject:model]) return model;
        NSMutableDictionary *newDic = [NSMutableDictionary new];
        [((NSDictionary *)model) enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
            NSString *stringKey = [key isKindOfClass:[NSString class]] ? key : key.description;//key.description必然字符串
            
            //保证 key 和 value 都存在。   然后再赋值。
            if (!stringKey) return;
...
+ (instancetype)modelWithJSON:(id)json {
    NSDictionary *dic = [self _yy_dictionaryWithJSON:json]; //json 转 dic
    return [self modelWithDictionary:dic];                  //dic 转模型
}

//---------
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    if (!dictionary || dictionary == (id)kCFNull) return nil;           //如果字典是空，return nil
    ...
    if ([one modelSetWithDictionary:dictionary]) return one;            //发生了 dic 转 model。
                                                    //如果是 YES，one 本身已经成功变成改类。  NO，return Nil。
    return nil;
}
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    //如果属性映射表 比 json字典数量多。  则遍历dic.然后遍历自己的 keypathMaps,multiMaps    不然则通过字典遍历所有映射
    if (modelMeta->_keyMappedCount >= CFDictionaryGetCount((CFDictionaryRef)dic)) {
        
        //除了 keypath 和 multiMaps ，都完成了 set 方法。  test bingo!!               dicValue
        CFDictionaryApplyFunction((CFDictionaryRef)dic, ModelSetWithDictionaryFunction, &context);
        
        //keypath 和 multiMap 都是没有 set 方法的。     依据 map 的 key，去找 dic里面对应的 value。效率极高，一找一个准。
...
//(CFArrayRef theArray, CFRange range, CFArrayApplierFunction CF_NOESCAPE applier, void *context);
        //此处用到遍历所有属性。
...
//-----------------copy encoder hash isequal description
- (id)modelCopy{
...
+ (NSDictionary *)modelDictionaryWithClass:(Class)cls dictionary:(NSDictionary *)dic {
    ...
    //"user1":{cls}   "user2":{no cls}  "user3":{name:{cls}}
    //三种情况只有第一种有效。感觉很有局限性。一个字典有很多 key:value。当 value 类型为 cls 时，帮助改字典转换。
