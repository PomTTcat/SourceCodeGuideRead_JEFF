...
//依据typeEncoding获取经过翻译的EncodingType     为什么要转换呢？
...
- (instancetype)initWithMethod:(Method)method {
    ...
    if (argumentCount > 0) {
        ...
        for (unsigned int i = 0; i < argumentCount; i++) {
            ...
            [argumentTypes addObject:type ? type : @""];    //有时候加@，有时候加：
            if (argumentType) free(argumentType);
            
        }
        _argumentTypeEncodings = argumentTypes;         //方法参数数组
    }
            ...
            case 'T': { // Type encoding
                if (attrs[i].value) {
                    ...
                    if ((type & YYEncodingTypeMask) == YYEncodingTypeObject) {
                        ...
                        //比如"@\"NSString\"" 实际上是@"NSString",len = 11.
                        if (len > 3) {
                            char name[len - 2];
                            name[len - 3] = '\0';                       //必须加上末尾的'\0'，不然不知道边界。
                            memcpy(name, attrs[i].value + 2, len - 3);  //从 N 开始读，总长度-3.
...
- (instancetype)initWithClass:(Class)cls {
    ...
    _superClassInfo = [self.class classInfoWithClass:_superCls];    //获取父类的ClassInfo 类信息
    return self;
}
...
+ (instancetype)classInfoWithClass:(Class)cls {
    ...
    dispatch_once(&onceToken, ^{
        classCache = CFDictionaryCreateMutable(CFAllocatorGetDefault(), 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);   //仅仅只是初始化
