...
# 性能分析用。如果执行时间少于0.1s，正常。如果大于0.1s，警告级别打印。
...
# 懒加载 连接。如果已经连接过了，则直接使用。如果没有，则获取全局变量的connect
# 给self.connection赋值
...
# global engine object: 已经成功的连接。
# engine = mysql.connector.connect(user='root', password='password', database='test', use_unicode=True)
engine = None
...
def insert(table, **kw):
    ...
    '''
    # 依据字典中的数量，生成对应个数的 %s,然后填入zip出来的对象。
